static void	draw_floor_pixel(t_context *context, t_point *floor_coord,
							int x, int y)
{
	if (!in_map(context->map, floor_coord->x, floor_coord->y) || x >= PROJ_WIDTH || y >= PROJ_HEIGHT)
		return ;
	context->screen_pixels[x + y * PROJ_WIDTH] = 
		*((t_uint *)context->surfaces[MAP_CHAR(context->map->ptr, floor_coord->x, floor_coord->y) == EXIT ? 
...
void		floorcaster(t_context *context, int x, t_polar_point *wall)
{
	...
	floor_height = (PROJ_HEIGHT - (int)(WALL_HEIGHT * PROJ_DIST / wall->dist)) / 2; 
